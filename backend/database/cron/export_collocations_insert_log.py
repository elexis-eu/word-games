...
mycursorDict.execute("SELECT * FROM admin_exports WHERE type = 'col_insert_log' and filename IS NULL AND started IS NULL LIMIT 1")
...
try:
    ...
    with open("../../exports/"+filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        mycursorDict.execute("SELECT * FROM collocation_log_insert WHERE created BETWEEN '" + new_export['date_from'].strftime("%Y-%m-%d") + " 00:00' AND '" + new_export['date_to'].strftime("%Y-%m-%d") + " 23:59'")
        col_log = mycursorDict.fetchall()

        writer.writerow(["Level", "Headword", "Structure", "Headword position", "Word1", "Score1", "ColID1", "Word2", "Score2", "ColID2", "Word3", "Score3", "ColID3", "User", "Created"])

        for log in col_log:
            print(log)
            writer.writerow([log['level'], log['headword'], structures[log['structure_id']]['name'], structures[log['structure_id']]['headword_position'],  log['word1'], log['score1'], log['col1'], log['word2'], log['score2'], log['col2'], log['word3'], log['score3'], log['col3'], log['user'], str(log['created'])])
