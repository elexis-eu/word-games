...
def delete_col(collocation_id):

        s_sql = "DELETE FROM collocation_priority WHERE collocation_id = " + collocation_id
        mycursor.execute(s_sql)

        s_sql = "DELETE FROM collocation_word WHERE collocation_id = " + collocation_id
        mycursor.execute(s_sql)

        s_sql = "DELETE FROM collocation WHERE id = " + collocation_id
        mycursor.execute(s_sql)

        mydb.commit()

mycursorDict.execute("SELECT id, filename FROM admin_imports WHERE type = 'col_remove' AND status = 'in_progress' LIMIT 1")
...
mycursorDict.execute("SELECT * FROM admin_imports WHERE type = 'col_remove' AND status = 'uploaded' LIMIT 1")
