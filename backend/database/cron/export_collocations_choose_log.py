...
mycursorDict.execute("SELECT * FROM admin_exports WHERE type = 'col_choose_log' AND filename IS NULL AND started IS NULL LIMIT 1")
...
try:
    ...
    with open("../../exports/"+filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        mycursorDict.execute("SELECT * FROM collocation_log_choose WHERE created BETWEEN '" + new_export['date_from'].strftime("%Y-%m-%d") + " 00:00' AND '" + new_export['date_to'].strftime("%Y-%m-%d") + " 23:59'")
        col_log = mycursorDict.fetchall()

        writer.writerow(["Level", "Structure", "Headword position", "Headword", "Word selected", "CollocationID", "Score", "User", "Created"])

        for log in col_log:
            print(log)
            writer.writerow([log['level'], structures[log['structure_id']]['name'], structures[log['structure_id']]['headword_position'],  log['headword'], log['word_selected'], log['col'], log['score'], log['user'], str(log['created'])])
