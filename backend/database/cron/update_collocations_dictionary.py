...
mycursorDict.execute("SELECT id, filename FROM admin_imports WHERE type = 'col_dict_update' AND status = 'in_progress' LIMIT 1")
...
mycursorDict.execute("SELECT * FROM admin_imports WHERE type = 'col_dict_update' AND status = 'uploaded' LIMIT 1")
...
try:
    ...
    for row in collocations:
        ...
        try:
            word_sql = "REPLACE INTO collocation (id, form, frequency, sailence, status, structure_id) VALUES (%s, %s, %s, %s, %s, %s)"
            ...
            word_sql = "REPLACE INTO collocation_priority (collocation_id, specific_weight, priority, total_weight, weight_limit, game_type) VALUES (%s, %s, 0, 0, 20, 1)"
